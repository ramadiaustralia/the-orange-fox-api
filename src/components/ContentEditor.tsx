...
import { Save, Languages, Clock, User, Plus, Trash2, FileEdit } from "lucide-react";
...
export default function ContentEditor({ page, items, onRefresh }: ContentEditorProps) {
  ...
  // Group by section and content_key
  const grouped: GroupedContent = {};
  items.forEach((item) => {
    if (!grouped[item.section]) grouped[item.section] = {};
    if (!grouped[item.section][item.content_key]) grouped[item.section][item.content_key] = {};
    grouped[item.section][item.content_key][item.locale as "en" | "id"] = item;
  });

  const startEdit = (section: string, key: string, en: string, id: string) => {
    const k = `${section}::${key}`;
    setEditing((prev) => ({ ...prev, [k]: { en, id } }));
  };

  const updateEdit = (section: string, key: string, locale: "en" | "id", value: string) => {
    const k = `${section}::${key}`;
    setEditing((prev) => ({
      ...prev,
  ...
  const cancelEdit = (section: string, key: string) => {
    const k = `${section}::${key}`;
    setEditing((prev) => {
      const n = { ...prev };
      delete n[k];
  ...
  const saveItem = async (section: string, key: string) => {
    const k = `${section}::${key}`;
    ...
    try {
      const pair = grouped[section]?.[key];
      // Save EN
      if (pair?.en) {
  ...
  const autoTranslate = async (section: string, key: string, from: "en" | "id") => {
    const k = `${section}::${key}`;
    const vals = editing[k];
    const sourceText = vals ? vals[from] : grouped[section]?.[key]?.[from]?.content_value;
    ...
    <div className="space-y-6">
      {sections.length === 0 && (
        <div className="text-center py-16 text-gray-500">
          <FileIcon className="mx-auto mb-3 text-gray-600" />
          <p className="text-lg font-medium">No content items yet</p>
          <p className="text-sm mt-1">Add your first content item for the <span className="text-orange font-medium">{page}</span> page</p>
        </div>
      )}

      {sections.map((section) => (
        <div key={section} className="bg-dark-400 border border-dark-50/50 rounded-2xl overflow-hidden">
          ...
          <div className="divide-y divide-dark-50/30">
            {Object.entries(grouped[section]).map(([contentKey, pair]) => {
              const k = `${section}::${contentKey}`;
              ...
              return (
                <div key={contentKey} className="p-6 hover:bg-dark-300/20 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <span className="text-sm font-semibold text-orange">{contentKey}</span>
                      {lastUpdate && (
                        <div className="flex items-center gap-3 mt-1 text-xs text-gray-600">
                          <span className="flex items-center gap-1"><Clock size={10} /> {new Date(lastUpdate).toLocaleDateString()}</span>
                          {updatedBy && <span className="flex items-center gap-1"><User size={10} /> {updatedBy}</span>}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        {isEditing && (
                          <button
                            onClick={() => autoTranslate(section, contentKey, "en")}
                      ...
                      {isEditing ? (
                        <textarea
                          value={enVal}
                          onChange={(e) => updateEdit(section, contentKey, "en", e.target.value)}
                          className="w-full bg-dark-200 border border-dark-50 text-white text-sm rounded-xl px-4 py-3 outline-none focus:border-orange focus:ring-1 focus:ring-orange/30 transition-all min-h-[80px] resize-y"
                        />
                      ) : (
                        <div
                          onClick={() => startEdit(section, contentKey, enVal, idVal)}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        {isEditing && (
                          <button
                            onClick={() => autoTranslate(section, contentKey, "id")}
                            disabled={isTranslating}
                            className="flex items-center gap-1 text-[10px] px-2 py-1 rounded-md bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-all disabled:opacity-50"
                          >
                            <Languages size={10} />
                      ...
                      {isEditing ? (
                        <textarea
                          value={idVal}
                          onChange={(e) => updateEdit(section, contentKey, "id", e.target.value)}
                          className="w-full bg-dark-200 border border-dark-50 text-white text-sm rounded-xl px-4 py-3 outline-none focus:border-orange focus:ring-1 focus:ring-orange/30 transition-all min-h-[80px] resize-y"
                        />
                      ) : (
                        <div
...
function FileIcon({ className }: { className?: string }) {
  return <FileEdit size={40} className={className} />;
}
